...
func (obj UtilDate) BuildDateFromString(d string) (DateInformation, error) {

	var format string = "020106150405.000"
	var err error
	var original time.Time

	original, err = time.Parse(format, d[:12]+"."+d[12:])

	return DateInformation{
		Error:     nil,
		Original:  original,
		Date:      obj.Format(original, "2020-01-02"),
		DateTime:  obj.FormatMilliseconds(original),
		TimeStamp: original.UnixNano() / (int64(time.Millisecond) / int64(time.Nanosecond)),
	}, err
}

func (obj UtilDate) BuildDateFromTimestamp(d int64) DateInformation {
	original := time.UnixMilli(d).UTC()
	return DateInformation{
		Error:     nil,
		Original:  original,
		Date:      obj.Format(original, "2020-01-02"),
